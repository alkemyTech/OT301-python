...
def transform_db():
    ...
    try:
        ...
        df_selec['birth_date'] = pd.to_datetime(df_selec.birth_date, dayfirst=True)

        df_selec['age'] = (df_selec['inscription_date'] - df_selec['birth_date']).astype('<m8[Y]')
        df_selec['age'] = df_selec.age.apply(
            lambda age: age + 100 if (age < 16) else age
        )
        df_selec = df_selec.drop(['birth_date','postal_code'], axis=1)
        df_selec = df_selec.applymap(str)
        df_selec['inscription_date'] = df_selec['inscription_date'].apply(lambda x: x.split(' ')[0])

        #Asignacion de codigo postal
        ...
        df_merge = pd.merge(left=df_selec,right=df_postal_code, how='left', left_on='location', right_on='location')
        df_merge['location'] = df_merge['location'].str.replace(" ", "_")

...
logging.basicConfig(format=f'%(asctime)s - GDUNTresDeFebrero_dag_etl - %(message)s', datefmt='%Y-%m-%d', level=logging.INFO)
...
def extract_db():
    try:
        with open(f'{HOME_DIR}/OT301-python/airflow/include/GDUNTresDeFebrero.sql', 'r') as sqlfile:
            query = sqlfile.read()
        pg_hook = PostgresHook('alkemy_db')
        pg_hook.copy_expert(f"COPY ({query}) TO STDOUT WITH CSV HEADER", filename=f'{HOME_DIR}/OT301-python/airflow/file/GDUNTresDeFebrero_select.csv')
...
def transform_db():
    try:
        df_select = pd.read_csv(f'{HOME_DIR}/OT301-python/airflow/file/GDUNTresDeFebrero_select.csv')
    ...
    try:
        df_merge.to_csv(f'{HOME_DIR}/OT301-python/airflow/datasets/GDUNTresDeFebrero_process.txt')
...
with DAG(dag_id='DD_GDUNTresDeFebrero_dag_etl',
    ...
    load = PythonOperator(
        ...
        op_kwargs={
            'filename': f'{HOME_DIR}/OT301-python/airflow/datasets/GDUNTresDeFebrero_process.txt',
            'key': f'GDUNTresDeFebrero_process.txt',
